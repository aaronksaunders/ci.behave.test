		...
		case "read":
			...
			if (opts && opts.query) {
				...
				if (model.id != undefined) {
					sql = sql + " WHERE " + model.idAttribute + "=?"
					rs = db.execute(sql, model.id);
				} else {
					rs = db.execute(sql);
				}
			...
			break;
		case "update":
			throw new Error("SHOULD NEVER BE HERE EXCEPTION");
			var names = [], values = [], q = [];
			for (var k in columns) {
				names.push(k + "=?");
				values.push(model.get(k));
...
function Migrate(Model) {
	...
	if (migrations.length)
		for (var i = 0; i < migrations.length; i++) {
			...
			migration(context);
			if (direction && context.id > targetNumber)
				break;
			if (!direction && context.id <= targetNumber)
				break;
